...
const findJoinTokensByTrustDomainName = `-- name: FindJoinTokensByTrustDomainName :many
SELECT j.id, j.trust_domain_id, j.token, j.used, j.expires_at, j.created_at, j.updated_at
FROM join_tokens j JOIN trust_domains td ON (j.trust_domain_id = td.id)
WHERE td.name = $1
ORDER BY j.created_at DESC
`

func (q *Queries) FindJoinTokensByTrustDomainName(ctx context.Context, name string) ([]JoinToken, error) {
	rows, err := q.query(ctx, q.findJoinTokensByTrustDomainNameStmt, findJoinTokensByTrustDomainName, name)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
