...
var (
	fakeFR = &common.FederationRelationship{
		Id: 1,
	}
	fakeFRJSON = `
...
func TestUpdateFederatedRelationshipConsent(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPut, fmt.Sprintf("/federation-relationships/%d", mockedId), strings.NewReader(fakeFRJSON))
