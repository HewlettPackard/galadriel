...
func LoadConfig(cmd *cobra.Command) (*harvester.Config, error) {
	...
	token, err := cmd.Flags().GetString("joinToken")
	if err != nil {
...
func init() {
	runCmd := NewRunCmd()
	runCmd.Flags().StringP("socketPath", "", "", "Path to bind the Galadriel Harvester API socket to")
	runCmd.Flags().StringP("config", "c", defaultConfigPath, "Path to the Galadriel Harvester config file")
	runCmd.Flags().StringP("joinToken", "t", "", "A join token generated by Galadriel Server")
